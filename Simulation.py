...
from lib.CZooming import CZoomingAlgorithm
...
class simulateOnlineData(object):
	...
	def batchRecord(self, iter_):
		print "Iteration %d"%iter_, "Pool", len(self.articles)," Elapsed time", datetime.datetime.now() - self.startTime
	...
	def runAlgorithms(self, algorithms):
		...
		for iter_ in range(self.testing_iterations):
			...
			for u in self.users:
				...
				OptimalReward, OptimalArticle = self.GetOptimalReward(u, self.articles) 
				OptimalReward += noise
							
				for alg_name, alg in algorithms.items():
					if(alg_name =="CZooming"):
						pickedArticle,ballPicked=alg.decide(self.articles, u.id, u.CoTheta)
					else:
					...
					#print('rwd',reward)
					if(alg_name =="CZooming"):
						alg.updateCounters(reward,ballPicked)
						alg.updateBall(ballPicked,u.CoTheta,pickedArticle)
					else:	
...
if __name__ == '__main__':
	...
	AM = ArticleManager(context_dimension+latent_dimension, n_articles=n_articles, ArticleGroups = ArticleGroups,FeatureFunc=featureUniform,  argv={'l2_limit':1})
	...
	if algName=='CZooming':
		T=n_users*testing_iterations
		algorithms['CZooming']= CZoomingAlgorithm(dimension=context_dimension, alpha=alpha, lambda_=lambda_,T=T)
